...
def cr_stacked_dbh_hist_tables(df, output_path):
...
def cr_stacked_ht_hist_tables(df, output_path):
    ...
    # Low 2004
    l4_counts = pd.DataFrame()
    l4_counts['size_bin_max'] = ht_range

    for i, row in l4_counts.iterrows():
        l4_counts.loc[i, '2004_cohort'] = low_df[(low_df['HT2004'] > (l4_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                 (low_df['HT2004'] <= l4_counts.loc[i, 'size_bin_max'])].shape[0]
    l4_counts.to_csv(output_path + 'ht_l4_for_hist.csv', index=False)

    # Low 2008
    l8_counts = pd.DataFrame()
    l8_counts['size_bin_max'] = ht_range
    for year in two_years:
        for i, row in l8_counts.iterrows():
            l8_counts.loc[i, '_'.join([str(year), 'cohort'])] = low_df[(low_df['first_year'] == year) &
                                                 (low_df['HT2008'] > (l8_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                 (low_df['HT2008'] <= l8_counts.loc[i, 'size_bin_max'])].shape[0]
    l8_counts.to_csv(output_path + 'ht_l8_for_hist.csv', index=False)

    # Low 2014
    l14_counts = pd.DataFrame()
    l14_counts['size_bin_max'] = ht_range
    for year in all_years:
        for i, row in l14_counts.iterrows():
            l14_counts.loc[i, '_'.join([str(year), 'cohort'])] = low_df[(low_df['first_year'] == year) &
                                                 (low_df['HT2014'] > (l14_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                 (low_df['HT2014'] <= l14_counts.loc[i, 'size_bin_max'])].shape[0]
    l14_counts.to_csv(output_path + 'ht_l14_for_hist.csv', index=False)
    ...
    # Mod 2004
    m4_counts = pd.DataFrame()
    m4_counts['size_bin_max'] = ht_range
    for i, row in m4_counts.iterrows():
        m4_counts.loc[i, '2004_cohort'] = mod_df[(mod_df['HT2004'] > (m4_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                 (mod_df['HT2004'] <= m4_counts.loc[i, 'size_bin_max'])].shape[0]
    m4_counts.to_csv(output_path + 'ht_m4_for_hist.csv', index=False)

    # Mod 2008
    m8_counts = pd.DataFrame()
    m8_counts['size_bin_max'] = ht_range
    for year in two_years:
        for i, row in m8_counts.iterrows():
            m8_counts.loc[i, '_'.join([str(year), 'cohort'])] = mod_df[(mod_df['first_year'] == year) &
                                                                       (mod_df['HT2008'] >
                                                                        (m8_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                                       (mod_df['HT2008'] <= m8_counts.loc[
                                                                           i, 'size_bin_max'])].shape[0]
    m8_counts.to_csv(output_path + 'ht_m8_for_hist.csv', index=False)

    # Mod 2014
    m14_counts = pd.DataFrame()
    m14_counts['size_bin_max'] = ht_range
    for year in all_years:
        for i, row in m14_counts.iterrows():
            m14_counts.loc[i, '_'.join([str(year), 'cohort'])] = mod_df[(mod_df['first_year'] == year) &
                                                                        (mod_df['HT2014'] > (
                                                                        m14_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                                        (mod_df['HT2014'] <= m14_counts.loc[
                                                                            i, 'size_bin_max'])].shape[0]
    m14_counts.to_csv(output_path + 'ht_m14_for_hist.csv', index=False)
    ...
    # Con 2004
    c4_counts = pd.DataFrame()
    c4_counts['size_bin_max'] = ht_range
    for i, row in m4_counts.iterrows():
        c4_counts.loc[i, '2004_cohort'] = con_df[(con_df['HT2004'] > (c4_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                 (con_df['HT2004'] <= c4_counts.loc[i, 'size_bin_max'])].shape[0]
    c4_counts.to_csv(output_path + 'ht_c4_for_hist.csv', index=False)

    # Con 2008
    c8_counts = pd.DataFrame()
    c8_counts['size_bin_max'] = ht_range
    for year in two_years:
        for i, row in c8_counts.iterrows():
            c8_counts.loc[i, '_'.join([str(year), 'cohort'])] = con_df[(con_df['first_year'] == year) &
                                                                       (con_df['HT2008'] > (
                                                                           c8_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                                       (con_df['HT2008'] <= c8_counts.loc[
                                                                           i, 'size_bin_max'])].shape[0]
    c8_counts.to_csv(output_path + 'ht_c8_for_hist.csv', index=False)

    # Con 2014
    c14_counts = pd.DataFrame()
    c14_counts['size_bin_max'] = ht_range
    for year in all_years:
        for i, row in c14_counts.iterrows():
            c14_counts.loc[i, '_'.join([str(year), 'cohort'])] = con_df[(con_df['first_year'] == year) &
                                                                        (con_df['HT2014'] > (
                                                                            c14_counts.loc[i, 'size_bin_max'] - 2.5)) &
                                                                        (con_df['HT2014'] <= c14_counts.loc[
                                                                            i, 'size_bin_max'])].shape[0]
    c14_counts.to_csv(output_path + 'ht_c14_for_hist.csv', index=False)
...
def cr_cv_table(df):
    cv_table = df.groupby('trt_clean').agg(
        {'DBH2014': coeff_var, 'HT2014': coeff_var, 'VOL2014': coeff_var}).reset_index()
